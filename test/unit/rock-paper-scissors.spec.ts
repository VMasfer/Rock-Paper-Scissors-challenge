...
describe('RockPaperScissors.sol', function () {
  ...
  describe('sellRPS(...)', function () {
    ...
    it('should revert if sending ether fails', async () => {
      let rpsPrice = await rockPaperScissors.rpsPrice();
      let amount = 3;
      await rockPaperScissors.connect(player1).buyRPS({ value: rpsPrice.mul(amount) });
      await rockPaperScissors.connect(deployer).withdrawEtherBalance(rpsPrice.mul(amount));
      await expect(rockPaperScissors.connect(player1).sellRPS(amount)).to.be.revertedWith('Failed to send ether');
    });
  });
